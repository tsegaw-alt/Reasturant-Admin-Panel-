...
export default {
  ...
  mounted () {
    ...
    if (this.customControls) {
      const close = this.$refs.customControlsRef.querySelector('[control=close]')
      close && close.addEventListener('click', this.closeWidget)
      const collapse = this.$refs.customControlsRef.querySelector('[control=collapse]')
      collapse && collapse.addEventListener('click', this.changeState.bind(this, null, 'collapse'))
      const expand = this.$refs.customControlsRef.querySelector('[control=expand]')
      expand && expand.addEventListener('click', this.changeState.bind(this, null, 'default'))
      const fullscreen = this.$refs.customControlsRef.querySelector('[control=fullscreen]')
      fullscreen && fullscreen.addEventListener('click', this.changeState.bind(this, null, 'fullscreen'))
      const restore = this.$refs.customControlsRef.querySelector('[control=restore]')
      restore && restore.addEventListener('click', this.changeState.bind(this, null, 'default'))
      const load = this.$refs.customControlsRef.querySelector('[control=load]')
